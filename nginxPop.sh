...
export REACT_APP_SERVER_DOMAIN="$REACT_APP_SERVER_DOMAIN"

# Use envsubst to replace variables directly in nginx.conf and create a temporary file
envsubst '$PORT $REACT_APP_PROXY_URI $REACT_APP_SERVER_URI $REACT_APP_SERVER_DOMAIN' < /etc/nginx/nginx.conf > /etc/nginx/nginx.conf.tmp
